...
打开 VS Code，点击左下角的人头图标，把你的 GitHub 账号登录到 VS Code 里，这样就能打开你的设置云同步。
...
请先安装此插件：**Unify Chat Provider**，安装好之后请重启 VS Code。
...
点击 VS Code 上方的输入框，输入 `unify chat provider`，选择内置供应商，这是 UCP 预先帮忙配置好的一些模型供应商。
...
1. 然后就是会要求你输入供应商名字，可以保持默认的 `DeepSeek`，按回车。
2. 然后粘贴你刚才复制的 API Key，然后再按回车。
...
如果你配置正确，右下角应该会有一个银行卡的图标，把鼠标挪上去会显示余额情况。
...
在设置里，输入 `chat.utilityModel`，然后把 `chat.utilityModel` 和 `chat.utilitySmallModel` 两个模型改成 dsv4p。
...
如果你想省钱的话，也可以在设置里输入 `explore agent`，然后启用代码研究子代理，然后把模型改成 dsv4p（dsv4f 也可以，更便宜更快）。
...
你可以拖拽“聊天”这两个字到左边的边栏，就可以显示到右边（我喜欢左边）。

然后再聊天栏中，在底下，选择模型，可以选 DeepSeek V4 Flash 验证一下你是不是正常连接到了。
...
前面的步骤都一样，在 UCP 的内置供应商里选择 NVIDIA，然后输入你的 API Key，按回车键。
...
然后就会有这个列表让你选择了，勾选之后点击确认，只推荐上图那几个模型。
...
按照下图的设置，修改 **连接超时**、**响应超时**、**最大重试次数**，然后点击 **返回** 到供应商配置，点击 **返回** 到供应商页面，然后点击 **保存**，然后就可以按 `Esc`，开始薅老黄羊毛。
